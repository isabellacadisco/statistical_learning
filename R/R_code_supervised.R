...
correlations <- sapply(numeric_columns, function(x) biserial.cor(x, df[[binary_column]], level = 1))
# level = 1 -> YES, R conta da 1!
correlations

# latex table
correlation_df <- data.frame(
  Variable = c("yob", "age", "age_arrival_ita", "years_in_ita", "years_of_education", 
               ...
               "cohabiting_persons_money_back_home"),
  Correlation = c(-0.16465706, 0.16465706, 0.06126307, 0.16004371, 0.09683370, 
...
# no strong correlations, variables likely to be useless to predict or explain the choice for prevention
...
model_0 = glm(prevention ~ genre, family = 'binomial', data = df)
summary(model_0)

# check how to go back to prob exactly
exp(model_0$coefficients[1])
exp(model_0$coefficients[2])
